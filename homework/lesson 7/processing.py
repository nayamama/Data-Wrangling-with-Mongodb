        ...
        for line in reader:
            ...
            for key in process_fields:
                if key == 'rdf-schema#label':
                    if '(' in line[key]:
                        rec[fields[key]] = line[key].strip().split(' ')[0]
                    elif line[key] == "NULL":
                        rec[fields[key]] = None
                    else:
                        rec[fields[key]] = line[key].strip()
                elif key == "name":
                    if line[key] == "NULL" or re.search(r'\W', line[key]):
                        rec[fields[key]] = rec["label"]
                    else:
                        rec[fields[key]] = line[key].strip()
                elif key == 'synonym':
                    if line[key] == "NULL":
                        ...
                        rec[fields[key]] = parse_array(line[key])
                    ...
                    if line[key] == "NULL":
                        ...
                        classification[fields[key]] = line[key].strip()
